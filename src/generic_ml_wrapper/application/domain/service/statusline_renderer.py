...
class StatuslineRenderer:
    """Renders the one-line status the client shows during a session."""

    def render_statusline(self, status: ClientStatus, workspace: Workspace) -> str:
    ...
    def _context(self, status: ClientStatus) -> str | None:
        ...
        if status.context_tokens is not None and status.context_window_size:
            ...
            return (
                f"ctx {self._compact(status.context_tokens)}/"
                f"{self._compact(status.context_window_size)} ({shown}%)"
            )
    ...
    def _compact(self, tokens: int) -> str:
        ...
        count is exact.
        """
        if tokens >= self._BILLION:
            return self._trim(tokens / self._BILLION) + "G"
        if tokens >= self._MILLION:
            return self._trim(tokens / self._MILLION) + "M"
        if tokens >= self._THOUSAND:
            return self._trim(tokens / self._THOUSAND) + "k"
        return str(tokens)
    ...
    def format_age(self, seconds: float) -> str:
        ...
        days, whole = divmod(whole, self._DAY)
        hours, whole = divmod(whole, self._HOUR)
        minutes, whole = divmod(whole, self._MINUTE)
        if days:
    ...
    def render_usage_row(  # noqa: PLR0913, PLR0917  (one row: label, id, turns, tokens, cost)
        self,
        label: str,
        name: str,
        turns: int,
        ...
        """
        head = f"{label} {name}" if age_s is None else f"{label} {name} ({self.format_age(age_s)})"
        parts = [head]
        if turns:
            parts.append(f"{turns} turns")
            parts.append(
                f"{self._compact(tokens)} tok"
            )  # k/M/G so a heavy job's total stays scannable
        parts.append(f"${cost_usd:.2f}")
        return "  " + " · ".join(parts)

    def _git(self, workspace: Workspace) -> str | None:
    ...
    def _folder(self, workspace: Workspace) -> str | None:
