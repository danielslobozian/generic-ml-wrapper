...
"""The outbound port for the user's roles: the folders under ``profile/roles/``."""
...
class RoleRepositoryPort(ABC):
    """Read and store the user's roles, each with the rules it holds.
    ...
    def find_all(self) -> tuple[Role, ...]:
        ...
        Returns:
            One :class:`Role` per folder, each carrying the rules it holds.
        """

    @abstractmethod
    def exists(self, role: Role) -> bool:
        """Whether a role with this code is already stored.

        Args:
            role: The role whose code to look for.
    ...
    def save(self, role: Role) -> None:
        """Store a role: its folder, its empty ``rules/`` drop-zone, and its sidecar.
