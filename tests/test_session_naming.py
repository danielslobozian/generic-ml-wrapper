...
def test_first_session_is_001() -> None:
    assert SessionNaming().next_session_id("JOB-1", []) == "JOB-1_001"


def test_next_is_one_past_the_highest() -> None:
    assert SessionNaming().next_session_id("JOB-1", ["JOB-1_001", "JOB-1_002"]) == "JOB-1_003"


def test_gaps_do_not_cause_collisions() -> None:
    assert SessionNaming().next_session_id("JOB-1", ["JOB-1_001", "JOB-1_005"]) == "JOB-1_006"


def test_ids_without_a_suffix_are_ignored() -> None:
    assert SessionNaming().next_session_id("JOB-1", ["weird", "JOB-1_002"]) == "JOB-1_003"
