...
def test_field_reads_a_bold_section_first_line() -> None:
    assert RuleParser().field(_RULE, "Rule") == "Never force-push a shared branch."
    assert RuleParser().field(_RULE, "Strength") == "hard"


def test_field_is_empty_for_an_absent_section() -> None:
    assert RuleParser().field(_RULE, "Signals") == ""


def test_field_collapses_whitespace() -> None:
    assert RuleParser().field("**Rule:**   be    careful.", "Rule") == "be careful."


def test_frontmatter_value_reads_a_key() -> None:
    assert RuleParser().frontmatter_value(_RULE, "status") == "active"
    assert RuleParser().frontmatter_value(_RULE, "name") == "no-force-push"


def test_frontmatter_value_is_empty_without_frontmatter() -> None:
    assert RuleParser().frontmatter_value("**Rule:** x.", "status") == ""


def test_is_draft_reads_the_status_key() -> None:
    assert RuleParser().is_draft(_RULE) is False
    assert RuleParser().is_draft(_RULE.replace("status: active", "status: draft")) is True
...
def test_is_draft_ignores_the_phrase_in_prose() -> None:
    ...
    assert RuleParser().is_draft(prose) is False
...
def test_the_seeded_template_is_active_not_draft() -> None:
    ...
    assert RuleParser().frontmatter_value(RULE_TEMPLATE, "status") == "active"
    assert RuleParser().is_draft(RULE_TEMPLATE) is False
