...
def _parser(tmp_path: Path) -> CursorStatusParser:
    """A parser whose plan cache is absent, which is the ordinary case."""
    return CursorStatusParser(tmp_path / "cursor-plan.json")
...
def test_cursor_gets_its_own_parser(tmp_path: Path) -> None:
    parsers = CataloguedStatusParsers(tmp_path / "cursor-plan.json")
    assert isinstance(parsers.for_client("cursor"), CursorStatusParser)
...
def test_every_other_client_gets_claudes_parser(tmp_path: Path) -> None:
    ...
    for client in ("claude", None, "codex"):
        assert isinstance(parsers.for_client(client), ClaudeStatusParser)
...
def test_the_cached_plan_is_folded_in_when_the_payload_has_none(tmp_path: Path) -> None:
    ...
    status = CursorStatusParser(cache).parse({"model": {"display_name": "Composer"}})
...
def test_a_payload_that_carries_its_own_plan_wins(tmp_path: Path) -> None:
    ...
    status = CursorStatusParser(cache).parse({"plan": {"auto_pct": 9, "api_pct": 8}})
...
def test_an_unreadable_cache_is_simply_no_plan(tmp_path: Path) -> None:
    ...
    assert CursorStatusParser(cache).parse({"model": {"display_name": "x"}}).extras == ()
