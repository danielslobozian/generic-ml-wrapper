...
"""Tests for PypiVersionChecker: PyPI JSON API reads, every failure degrading to None."""
...
def test_returns_the_version_on_a_normal_response(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert PypiVersionChecker().latest_version("generic-ml-wrapper") == "1.2.3"
...
def test_network_failure_degrades_to_none(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert PypiVersionChecker().latest_version("generic-ml-wrapper") is None
