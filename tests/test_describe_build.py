...
def test_a_released_build_is_named_by_its_stamp() -> None:
    assert DescribeBuildService(_Stamp("2026.08.04-1")).execute() == (
...
def test_an_unbuilt_checkout_says_so() -> None:
    ...
    assert DescribeBuildService(_Stamp(None)).execute() == f"gmlw {__version__} (source, unbuilt)"


def test_the_line_is_one_line() -> None:
    assert "\n" not in DescribeBuildService(_Stamp("x")).execute()
