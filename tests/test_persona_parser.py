...
def test_parses_frontmatter_and_body() -> None:
    ...
    persona = PersonaParser().parse_persona("fallback", text)
...
def test_name_falls_back_to_the_file_stem() -> None:
    persona = PersonaParser().parse_persona("mentor", "---\ndescription: Guide.\n---\nBody.")
...
def test_no_frontmatter_is_all_body() -> None:
    persona = PersonaParser().parse_persona("plain", "Just a tone block, no metadata.")
...
def test_single_quotes_and_blank_lines_are_handled() -> None:
    persona = PersonaParser().parse_persona(
        "x", "---\nname: 'terse'\n\ndescription: 'Short.'\n---\nB"
    )
